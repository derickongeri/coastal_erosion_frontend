...
// post request
const requestAnalysis = async ({ payload, uri, caller }) => {
  try {
    ...
    if (response.data.error || response.data.success === "false")
      ...
      if(response.data.message) return positive({
        message:response.data.message,
        color:'green',
        position: 'center'
      })
    setAnalysisResults(response.data); // set the analysis results to the store
...
const processGeometryPayload = () => {
 ...
 const {geojson, name, ...geometry_request_payload} =getGeometryData.value
  return {
    // admin_level: getGeometryData.value?.admin_level,
    custom_coords: custom ? custom : undefined,
    // vector: getGeometryData.value?.vector,
    // admin_0: getGeometryData.value?.vector,
